"""Antivenom experiment runner — 4-ply variant of E3 (black plays fangs).

Uses the shorter ``black_plays_fangs_4_ply.pgn`` spec (white has only two
wildcard plies instead of three) so each top-N result has at most one
transposition (m1m2 ↔ m2m1) for any non-checking move pair. Useful for
exercising the dedup + per-ply eval display end-to-end without paying for the
full 3-wildcard tree.
"""
...
# Make `src` importable when running this file directly (e.g. `python experiments/E4_*.py`).
...
# CONFIGURATION
config_experiment_tag: str = "E4_black_plays_fangs_short"
config_input_path: Path = Path.cwd().parent / "inputs" / "format_0" / "black_plays_fangs_4_ply.pgn"
config_output_dir: Path = Path.cwd().parent / "outputs" / config_experiment_tag
...
for i, r in enumerate(result.results[:config_num_solutions]):
    ...
    trans_note: str = f" (+{n_trans} transposition{'s' if n_trans != 1 else ''})" if n_trans else ""
